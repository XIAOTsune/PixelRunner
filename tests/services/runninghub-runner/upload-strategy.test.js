...
function buildFakePngBuffer(byteLength = 32) {
  const size = Math.max(12, Number(byteLength) || 12);
...
function installFakeImageResizeEnv(options = {}) {
  const previousDocument = global.document;
  const previousImage = global.Image;
  const previousURL = global.URL;
  const imageWidth = Math.max(1, Number(options.imageWidth) || 4000);
  const imageHeight = Math.max(1, Number(options.imageHeight) || 2000);
  const bytesPerPixel = Number(options.bytesPerPixel) > 0 ? Number(options.bytesPerPixel) : 1;
    ...
    createElement: (tag) => {
      ...
      return {
        ...
        toBlob(callback, mimeType, quality) {
          const format = String(mimeType || "image/png").toLowerCase();
          const qualityNum = Number(quality);
          const qualityScale = Number.isFinite(qualityNum) ? Math.max(0.15, Math.min(1, qualityNum)) : 1;
          const formatScale = format === "image/webp" ? 0.5 : format === "image/jpeg" ? 0.6 : 1;
          const size = Math.max(1, Math.round(this.width * this.height * bytesPerPixel * formatScale * qualityScale));
          callback(new Blob([new Uint8Array(size)], { type: format }));
        }
      };
    }
  };
...
test("uploadImage applies uploadMaxEdge cap before upload", async () => {
  const restoreEnv = installFakeImageResizeEnv({
    imageWidth: 4000,
    imageHeight: 2000,
    bytesPerPixel: 1
  });
    ...
    const result = await uploadImage(
      ...
      },
      {
        fetchImpl: async (_url, init) => {
          uploadedBlob = init.body.get("file");
          return { ok: true, status: 200, payload: { code: 0, data: { fileName: "token-cap" } } };
        },
        parseJsonResponse: async (response) => response.payload
      }
    );

    assert.equal(result.value, "token-cap");
    assert.ok(uploadedBlob instanceof Blob);
    assert.equal(uploadedBlob.size, 1024 * 512);
    assert.ok(logs.some((entry) => /Image resized before upload: 4000x2000 -> 1024x512/.test(entry.line)));
  } finally {
    restoreEnv();
  }
});

test("uploadImage auto-compresses image when size is above 9MiB", async () => {
  const restoreEnv = installFakeImageResizeEnv({
    imageWidth: 5000,
    imageHeight: 5000,
    bytesPerPixel: 1
  ...
  try {
    const result = await uploadImage(
      "api-key",
      buildFakePngBuffer(12 * 1024 * 1024),
      {
        uploadMaxEdge: 0,
        log: (line, level) => logs.push({ line, level })
      },
      {
        fetchImpl: async (_url, init) => {
          uploadedBlob = init.body.get("file");
          return { ok: true, status: 200, payload: { code: 0, data: { fileName: "token-compressed" } } };
        },
        parseJsonResponse: async (response) => response.payload
      }
    );

    assert.equal(result.value, "token-compressed");
    assert.ok(uploadedBlob instanceof Blob);
    assert.ok(uploadedBlob.size <= 9 * 1024 * 1024);
    assert.ok(logs.some((entry) => /Image auto-compressed by quality:/.test(entry.line)));
  } finally {
    restoreEnv();
  }
});
