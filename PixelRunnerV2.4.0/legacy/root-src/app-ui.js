(function initUiModule(global) {
  ...
  function bindTactileFeedback() {
    const tactileTargets = document.querySelectorAll(
      ".ghost-btn, .mini-btn, .secondary-btn, .primary-btn, .nav-tab"
    );
  ...
  function bindToolActions() {
    ...
    const pendingToolButtons = [
      "btnContentAwareFill",
      "btnSelectAndMask"
    ];

    pendingToolButtons.forEach((id) => {
      const button = modules.runtime.getById(id);
      if (!button) return;

      button.addEventListener("click", () => {
        logToWorkspace("这个工具按钮下一步会接入更稳定的 Photoshop 动作，目前先保留占位。", "warn");
      });
    });
  }
