(function initWorkspaceModule(global) {
  ...
  function getSelectedCaptureAsset() {
    ...
    const selected =
      assets.find((asset) => String(asset.assetId || "") === selectedAssetId) ||
      assets[0] ||
      null;
  ...
  function buildAutoPlacementPayload(result) {
    const sourceDocument = result && result.sourceDocument && typeof result.sourceDocument === "object"
  ...
  async function autoPlaceLastResult() {
    ...
    const placementSummary = sourceDocument && sourceDocument.selectionBounds
      ? `Auto-placed back using source selection ${formatSelectionLabel(sourceDocument.selectionBounds)}`
      : "Auto-placed back into the source document";
