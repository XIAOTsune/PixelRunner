var PixelRunnerHostBundle = (() => {
  ...
  async function createTransparentTempDocument(app, action, name, size, resolution) {
    ...
    if (app && app.documents && typeof app.documents.add === "function") {
      try {
        const doc2 = await app.documents.add({
          width,
          height,
  ...
  async function renameActiveLayer2(action, layerName) {
    await action.batchPlay([{
      _obj: "set",
      _target: [{ _ref: "layer", _enum: "ordinal", _value: "targetEnum" }],
      to: {
  ...
  async function createGlowLayerFromDocument(config, app, document2, action, saveOptions = {}, options = {}) {
    ...
    try {
      ...
      const resizeInfo = previewMaxEdge > 0 ? await tempOp("Resize glow temp document", () => resizeDocumentToLongEdge(action, tempDoc, previewMaxEdge)) : { scale: 1, width: sourceSize.width, height: sourceSize.height };
  ...
  async function setActiveLayerStyle2(action, options = {}) {
  ...
  async function placeImageFromUrl(payload) {
    ...
    const pngAlphaUsesPartialCanvas = Boolean(
      pngInfo && pngAlphaBounds && !dimensionsNearlyMatch(
    ...
    await core.executeAsModal(async () => {
      ...
      if (preserveCanvasBounds) {
        placementMode = normalizedMode === "stretch" ? "stretch" : normalizedMode === "cover" ? "cover" : normalizedMode === "original" || normalizedMode === "pixel-perfect" ? "original" : "contain";
  ...
  var blankImageTokenCache = /* @__PURE__ */ new Map();
  var BLANK_IMAGE_PNG_BASE64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO2fJ0QAAAAASUVORK5CYII=";
  function normalizeAppId2(value) {
  ...
  async function uploadImageValue(apiKey, imageValue, settings = {}) {
    ...
    const fallbackMime = imageValue && typeof imageValue === "object" && String(imageValue.mimeType || "").trim() || "image/jpeg";
    ...
    const timeoutMs = Math.max(5e3, Number(settings.timeout || 180) * 1e3);
    const endpoints = [
  ...
  async function getBlankImageToken(apiKey, settings = {}) {
    ...
    const pending = uploadImageValue(cacheKey, BLANK_IMAGE_PNG_BASE64, settings).then((token) => {
  ...
  async function buildSubmissionInputs(app, inputValues, apiKey, settings = {}) {
    ...
    for (let index = 0; index < inputs.length; index += 1) {
      ...
      if (input && input.fieldData !== void 0 && !isImageInput) {
  ...
  async function fetchTextWithTimeout(url, options = {}, timeoutMs = 3e4) {
    const controller = typeof AbortController !== "undefined" ? new AbortController() : null;
    const timer = controller ? setTimeout(() => {
      try {
        controller.abort();
...
${extraRequirement}`);
  ...
  async function runAiOptimizeInternal(payload = {}) {
    const apiKey = String(payload.apiKey || "").trim();
    const appId = normalizeAppId2(payload.appId);
    ...
    const textContent = String(await fetchTextWithTimeout(txtCandidate.url, {}, Math.max(1e4, Number(settings.timeout || 180) * 1e3))).trim();
    if (!textContent) {
  ...
  function isParameterShapeError(message) {
    ...
    return marker.includes("webappid cannot be null") || marker.includes("param apikey is required") || marker.includes("param api key is required");
  }
  ...
  async function submitRunningHubTask(args = []) {
    ...
    const settings = payload.settings && typeof payload.settings === "object" ? payload.settings : {};
    const apiKey = String(payload.apiKey || "").trim();
    const appId = normalizeAppId2(app.appId || payload.appId);
    if (!apiKey) throw new Error("RunningHub API Key is missing");
  ...
  async function runAiOptimizeTask(args = []) {
    const payload = args && args[0] && typeof args[0] === "object" ? args[0] : {};
    return runAiOptimizeInternal(payload);
  }

  // src/host/shell.js
  var TUTORIAL_RELATIVE_PATH = ["pages", "runninghub-guide.html"];
  ...
  function joinNativePath(basePath, segments = []) {
    ...
    const cleanSegments = (Array.isArray(segments) ? segments : []).map((segment) => String(segment || "").trim().replace(/^[\\/]+|[\\/]+$/g, "")).filter(Boolean);
  ...
  async function resolveTutorialPath() {
    try {
  ...
  async function openLocalPath(args = []) {
