...
assert.doesNotMatch(localUpscaleWebview, /分块 \$\{completed\}/);
...
assert.match(photoshopService, /async function normalizeLocalUpscaleTempDocumentBitDepth/);
assert.match(photoshopService, /_obj: "convertMode",\s+depth: 8,\s+merge: false/);
assert.match(photoshopService, /await normalizeLocalUpscaleTempDocumentBitDepth\(action, tempDoc\)/);
