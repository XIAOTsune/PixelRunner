...
import { blendMatchActiveLayer, hydrateBlendMatchPreviewPlan, previewBlendMatchActiveLayer, previewBlendMatchSamplesActiveLayer } from "./blend-match.js";
import {
...
function getGlowConfig(payload = {}) {
  ...
  const radius = opticalStyle
    ? clampNumber(style === "starburst" ? payload.starLength : payload.streakLength, 12, 240, style === "starburst" ? 58 : 86)
    : clampNumber(payload.radius, 1, 240, 81);
  ...
  const highlightFuzziness = opticalStyle
    ? clampNumber(Math.round((style === "starburst" ? 8 : 10) + (100 - threshold) * 0.08), 6, 20, 10)
  ...
  const sourceGamma = Number((opticalStyle
    ? clampNumber(0.72 + threshold / 100 * 0.34, 0.68, 1.18, 0.86)
    : clampNumber(0.68 + thresholdGate / 100 * 0.72 + fade / 180 - brightnessLift * 0.08, 0.48, 1.75, 0.92)).toFixed(2));
  ...
  switch (actionName) {
    ...
    case "saturationObserverLayer": {
      ...
      const selectiveAdjustment = colorRanges.map(([color, black]) => ({
        _obj: "selectiveColorAdjustmentV2",
        cyan: 0,
        magenta: 0,
        yellow: 0,
        black,
      ...
      const result = await action.batchPlay([
        ...
        },
        {
          ...
          to: {
            ...
            method: { _enum: "method", _value: "absolute" },
...
export async function runToolActionByName(payload, context) {
  ...
  if (actionName === "blendMatchPreviewPlan") {
    return hydrateBlendMatchPreviewPlan(payload, context);
  }
