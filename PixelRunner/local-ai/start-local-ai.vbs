...
If IsServiceCompatible() Then WScript.Quit 0
StopCurrentPixelRunnerService

If HasCommand("pyw") Then
  shell.Run "pyw -3 " & Quote(serverPath), 0, False
ElseIf HasCommand("pythonw") Then
  shell.Run "pythonw " & Quote(serverPath), 0, False
ElseIf HasCommand("py") Then
  shell.Run "py -3 " & Quote(serverPath), 0, False
ElseIf HasCommand("python") Then
  shell.Run "python " & Quote(serverPath), 0, False
End If
...
Function IsServiceCompatible()
  ...
  request.Open "GET", "http://127.0.0.1:17836/v1/health", False
  request.Send
